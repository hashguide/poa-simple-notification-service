...
log4js.configure({
    appenders: { dequeue: { type: 'file', filename: '$HOME/poa-simple-notification-service/dequeue.log' } },
...
q.on('next',function(task) {
    try {
        ...
        config = yaml.safeLoad(fs.readFileSync('$HOME/poa-simple-notification-service/email-local.yaml', 'utf8'));
